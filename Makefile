# This Makefile is for the Linux::MemInfo extension to perl.
#
...
#     ABSTRACT_FROM => q[MemInfo.pm]
#     AUTHOR => q[Chad Kerner <ckerner@amdocs.com>]
#     NAME => q[Linux::MemInfo]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[MemInfo.pm]
...
AR_STATIC_ARGS = cr
NAME = Linux::MemInfo
DISTNAME = Linux-MemInfo
NAME_SYM = Linux_MemInfo
VERSION = 0.01
VERSION_SYM = 0_01
XS_VERSION = 0.01
...
FULLEXT = Linux/MemInfo
BASEEXT = MemInfo
PARENT_NAME = Linux
DLBASE = $(BASEEXT)
VERSION_FROM = MemInfo.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
...
# Where to put things:
INST_LIBDIR      = $(INST_LIB)/Linux
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Linux
...
TO_INST_PM = MemInfo.pm

PM_TO_BLIB = MemInfo.pm \
...
manifypods : pure_all MemInfo.pm
	@$(POD2MAN) \
	MemInfo.pm \
	$(INST_MAN3DIR)/Linux::MemInfo.$(MAN3EXT)
...
realclean purge ::  clean
	...
	rm -rf $(DISTVNAME)
	rm -f  blib/lib/Linux/MemInfo.pm
...
pm_to_blib: $(TO_INST_PM)
	...
	-e "pm_to_blib({qw{MemInfo.pm blib/lib/Linux/MemInfo.pm}},'$(INST_LIB)/auto','$(PM_FILTER)')"
	@$(TOUCH) $@
